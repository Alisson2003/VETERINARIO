...
const Chat = () => {
    ...
    const handleEnterChat = (data) => {
        ...
        setResponses((prev) => [
            ...prev,
            {
                body: `¡Bienvenido a SmartVet, ${data.name}!`,
                from: "Sistema",
    ...
    useEffect(() => {
        ...
        newSocket.on("enviar-mensaje-front-back", (payload) => {
            ...
            // Evita duplicar tu propio mensaje
            if (payload.from === nameUser) return;
            setResponses((prev) => [...prev, payload]);
        })
                            ...
                            <div className="flex flex-col space-y-4 p-3 overflow-y-auto scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch">

                                {
                                    responses.map((response, index) => (
                                        <div
                                        key={index}
                                        className={`my-2 p-4 text-sm rounded-md text-white max-w-xs ${
                                            response.from === nameUser ? 'bg-slate-700 self-end' : 'bg-black self-start'
                                        }`}
                                        >
                                        <div className="font-semibold mb-1">{response.from}</div>
